...
int LibUSBDevice::Open(std::function<void(USBEvent event, uint8_t *buf, size_t size)> usbEventCallback)
{
    ASTRA_LOG;
...
int LibUSBDevice::Write(uint8_t *data, size_t size, int *transferred)
{
    ASTRA_LOG;
...
uint16_t LibUSBDevice::GetProductId() const
{
    if (m_device == nullptr) return 0;
    libusb_device_descriptor desc;
    int ret = libusb_get_device_descriptor(m_device, &desc);
...
uint8_t LibUSBDevice::GetNumInterfaces() const
{
    if (m_config == nullptr) return 0;
...
void LibUSBDevice::HandleTransfer(struct libusb_transfer *transfer)
{
    ...
    LibUSBDevice *device = static_cast<LibUSBDevice*>(transfer->user_data);

    bool resubmit = false;

    auto queueEvent = [device](USBDevice::USBEvent event, uint8_t *buf, size_t size) {
